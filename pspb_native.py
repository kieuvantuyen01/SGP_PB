...
def exactly_k(var: List[int], k):
    ...
    # Initialize PBEnc with the default configuration
    pbenc = PBEnc()

    # Use the equals method to encode the exactly-k constraint
    # Assuming weights are all 1 if not specified otherwise
    weights = [1] * len(var)  # This line can be omitted if weights are indeed all 1
    top_id = id_variable + 1

    # Encode the exactly-k constraint
    clauses = pbenc.equals(lits=var, weights=weights, bound=k, top_id=top_id, encoding=6)

    print(clauses)

    # Check if the encoding is the native MiniCard encoding, which produces a single clause
    if isinstance(clauses, tuple) and len(clauses) == 2 and isinstance(clauses[0], list) and isinstance(clauses[1], int):
        # For MiniCard native encoding, clauses is a tuple ([lits], k)
        # Directly add the literals as a single clause
        plus_clause(clauses[0])
    else:
        # For other encodings, clauses is a list of clauses
        # Add constraints by calling the plus_clause function for each clause
        for clause in clauses:
            plus_clause(clause)

    # Update the global variable id_variable based on the new variables introduced by the encoding
    id_variable = max(id_variable, max(max(abs(lit) for lit in clause) for clause in clauses if isinstance(clause, list)))

    # Optionally, print the new max variable ID
    # print(f"Max var: {id_variable}")
...
def write_to_cnf(num_vars, num_clauses, problem_name):
    ...
    input_path = online_path + "input_cnf/PSPB_native"
...
def run_kissat(problem_name):
    ...
    bashCommand = f"ls input_cnf/PSPB_native/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed > output_kissat/{problem_name}.txt"
