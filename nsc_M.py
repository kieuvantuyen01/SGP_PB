            ...
            for golfer1 in range(1, num_players + 1):
                for golfer2 in range(golfer1 + 1, num_players + 1):
                        if ((golfer1 - 1) // players_per_group == (golfer2 - 1) // players_per_group):
                            plus_clause([-get_variable(golfer1, group, week), -get_variable(golfer2, group, week)])
...
def run_kissat(problem_name):
    ...
    def write_to_cnf():
        ...
        input_path = online_path + "input_cnf/nsc_M"
    ...
    print_to_console_and_log("Searching for a solution...")
    bashCommand = f"ls input_cnf/nsc_M/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed" # > {file_path}"
