...
def exactly_k(var: List[int], k):
    ...
    pbConfig = PBConfig()
    pbConfig.set_PB_Encoder(pblib.PB_BINARY_MERGE)
...
def write_to_cnf(num_vars, num_clauses, problem_name):
    ...
    input_path = online_path + "input_cnf/PB_BINARYMERGE"
...
def run_kissat(problem_name):
    ...
    bashCommand = f"ls input_cnf/PB_BINARYMERGE/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed > output_kissat/{problem_name}.txt"
