...
def exactly_k(var: List[int], k):
    ...
    card_enc = CardEnc.equals(lits=var, bound=k, top_id=top_id, encoding=5)
...
def write_to_cnf(num_vars, num_clauses, problem_name):
    ...
    input_path = online_path + "input_cnf/Pysat_ladder"
...
def run_kissat(problem_name):
    ...
    bashCommand = f"ls input_cnf/Pysat_ladder/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed > output_kissat/{problem_name}.txt"
