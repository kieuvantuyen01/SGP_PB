...
def exactly_k(var: List[int], k):
    ...
    # Assuming id_variable is used to track the maximum variable ID used so far
    top_id = id_variable + 1 if id_variable is not None else max(var) + 1

    # Create a CardEnc object
    card_enc = CardEnc.equals(lits=var, bound=k, top_id=top_id, encoding=8)

    # The CardEnc.equals method returns a list of clauses
    formula = card_enc.clauses
    ...
    if card_enc.nv > top_id:
        id_variable = card_enc.nv
    else:
        id_variable = top_id + len(formula)  # Adjust based on your ID management strategy
...
def write_to_cnf(num_vars, num_clauses, problem_name):
    ...
    input_path = online_path + "input_cnf/Pysat_kmtotalizer"
...
def run_kissat(problem_name):
    ...
    bashCommand = f"ls input_cnf/Pysat_kmtotalizer/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed > output_kissat/{problem_name}.txt"
