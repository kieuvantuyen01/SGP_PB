...
def run_kissat(problem_name):
    # Create the directory if it doesn't exist
    input_path = online_path + "output_kissat/matrix"
    if not os.path.exists(input_path): os.makedirs(input_path)
    ...
    bashCommand = f"ls input_cnf/matrix/{problem_name}.cnf | xargs -n 1 ./kissat --time={time_budget} --relaxed > output_kissat/{problem_name}.txt"
